...
def load_csv(path: str):
    """Load CSV robustly (comma or tab delimited)."""
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        sample = f.read(4096)
        f.seek(0)
        # Heuristic: Excel exports sometimes come as TSV.
        delimiter = ","
...
def load_geojson_any():
    """Loads SA2 GeoJSON from data folder.

    We intentionally prefer the plain .geojson on GitHub/Render because
    editing/renaming large files in the GitHub web UI can accidentally truncate
    the contents (resulting in 0-2 byte files). Uploading the .geojson directly
    into /data is the safest workflow.
    """
    if os.path.exists(GEOJSON_PATH):
        with open(GEOJSON_PATH, "r", encoding="utf-8") as f:
            return json.load(f)

    # Backwards-compatible: accept gzip if present.
...
def build_map(metric: str):
    ...
    # Plotly Express depends on pandas; use graph_objects to avoid pandas.
    fig = go.Figure(
        go.Choroplethmapbox(
            geojson=GEOJSON,
            locations=[d["SA2_CODE21"] for d in df_map],
            z=[d["Metric_Value"] for d in df_map],
            featureidkey=FEATURE_ID_KEY,
            colorscale="Turbo",
            # Choroplethmapbox does not accept a top-level `opacity`; use marker.opacity.
            marker=dict(opacity=0.80, line=dict(width=1, color="rgba(255,255,255,0.65)")),
            customdata=customdata,
            hovertemplate=None,  # set below
            colorbar=dict(title="", ticks="outside"),
            showscale=True,
        )
    )
    ...
    fig.update_layout(
        ...
        paper_bgcolor="white",
        plot_bgcolor="white",
        # Colorbar configured on the trace (no global coloraxis).
    )
                    ...
                    dbc.Card(
                        dbc.CardBody(
                            [
                                ...
                                html.Div(
                                    [
                                        html.Span("Ready — select a metric, then click Update", id="status-text"),
                                        html.Span("", id="status-running", className="status-running"),
                                    ],
...
def update(n_clicks_update, n_clicks_reset, metric):
    ...
    if trigger == "btn-reset":
        ...
        return (
            build_map(m),
            build_metric_bar(m),
            build_npv_netincome_scatter(),
            build_summary_cards(build_df_map(ROWS, m), m),
            "Ready (cache reset)",
            _now(),
        )

    if not metric:
        return no_update, no_update, no_update, no_update, "Ready", _now()

    # Build figures (cached)
    fig_map = build_map(metric)
    fig_bar = build_metric_bar(metric)
    fig_scatter = build_npv_netincome_scatter()
    summary = build_summary_cards(build_df_map(ROWS, metric), metric)

    return fig_map, fig_bar, fig_scatter, summary, "Updated", _now()
...
# Client-side (browser) status indicator that flips to "Updating…" while any output is loading.
app.clientside_callback(
    """
    function(ls_map, ls_bar, ls_scat, ls_tbl) {
        const arr = [ls_map, ls_bar, ls_scat, ls_tbl];
        const loading = arr.some((x) => x && x.is_loading);
        return loading ? "Updating…" : "";
    }
    """,
    Output("status-running", "children"),
    Input("fig-map", "loading_state"),
    Input("fig-bar", "loading_state"),
    Input("fig-scat", "loading_state"),
    Input("tbl", "loading_state"),
)
