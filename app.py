...
def build_point_map(metric: str) -> go.Figure:
    ...
    if not lats:
        ...
        fig.update_layout(
            ...
            title="Map — no coordinates available",
        )
        _CACHE[cache_key] = fig
        return fig

    lats, lons = _jitter_points(lats, lons)
    ...
    fig = go.Figure(
        go.Scattermapbox(
            ...
            hovertemplate=(
                ...
                f"{metric_label(metric)}: %{{marker.color:,.2f}}<br>"
                "NPV (5Y): %{customdata[5]:,.0f}<br>"
                "Net Income (pa): %{customdata[6]:,.0f}<extra></extra>"
            ),
        )
    )
    ...
    fig.update_layout(
        template="plotly_dark",
        title=f"{metric_label(metric)} — map of 11 store points",
...
def build_scatter_npv_net_income() -> go.Figure:
    ...
    for r in ROWS:
        ...
        xs.append(float(npv))
        ys.append(float(ni))
        # Shorten labels a bit to reduce overlap
        short = nm
        if len(short) > 22:
            short = short[:19] + "…"
        txt.append(short)
    ...
    textpos = [positions[i % len(positions)] for i in range(len(txt))]
            ...
            [
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody(
                            dcc.Loading(
                                dcc.Graph(id="map", config={"displayModeBar": True, "scrollZoom": True}),
                                type="default",
                            )
                        ),
                ...
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody(
                            dcc.Loading(
                                dcc.Graph(id="bar", config={"displayModeBar": True}),
                                type="default",
                            )
                        ),
                        className="panel-card",
