...
def load_geojson(path: Path) -> dict:
    # Robust read: handle UTF-8 BOM and avoid crashing on empty/missing files.
    if not path.exists():
        raise FileNotFoundError(f"GeoJSON file not found: {path}")
    raw = path.read_bytes()
    if len(raw) < 10:
        raise ValueError(f"GeoJSON file looks empty: {path} (bytes={len(raw)})")
    ...
    try:
        ...
        # Fallback: latin-1 decode (should rarely happen)
