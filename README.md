...
- Loads SA2 polygons for Greater Sydney from `data/sa2_greater_sydney_2021.geojson.gz`
- Lets you pick a metric and renders a choropleth map (only the 11 SA2s are colored)
- Shows an interactive table (sort + filter) for all 11 locations

## Important: moving/renaming the GeoJSON on GitHub
Do **NOT** rename/move large `.geojson` files using GitHub's **web editor** — it often saves an empty file (2 bytes),
which breaks Render deploy.
